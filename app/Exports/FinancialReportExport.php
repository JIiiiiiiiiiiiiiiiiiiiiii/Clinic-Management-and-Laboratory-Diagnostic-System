...
class FinancialReportExport implements WithMultipleSheets, WithEvents
{
    ...
    public function sheets(): array
    {
        ...
        $summaryRows = [
            ...
            ['Date', $this->date],
            ['Total Revenue', 'PHP ' . number_format($this->data['summary']['total_revenue'] ?? 0, 2)],
            ['Total Transactions', $this->data['summary']['total_transactions'] ?? 0],
            ['Cash Total', 'PHP ' . number_format($this->data['summary']['cash_total'] ?? 0, 2)],
            ['HMO Total', 'PHP ' . number_format($this->data['summary']['hmo_total'] ?? 0, 2)],
            ['Pending Amount', 'PHP ' . number_format($this->data['summary']['pending_amount'] ?? 0, 2)],
        ];
        ...
        foreach ($this->transactions as $transaction) {
            $transactionRows[] = [
                ...
                'PHP ' . number_format($transaction->original_amount ?? $transaction->total_amount, 2),
