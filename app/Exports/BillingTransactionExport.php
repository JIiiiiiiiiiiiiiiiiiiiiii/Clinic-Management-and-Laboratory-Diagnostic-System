...
class BillingTransactionExport implements WithMultipleSheets, WithEvents
{
    ...
    protected function getSummaryData(): array
    {
        ...
        return [
            ...
            ['CANCELLED AMOUNT', '₱' . number_format($cancelledAmount, 2)],
            ['', ''],
            ['CASH PAYMENTS', '₱' . number_format($cashAmount, 2)],
            ['HMO PAYMENTS', '₱' . number_format($hmoAmount, 2)],
            ['CARD PAYMENTS', '₱' . number_format($cardAmount, 2)],
            ['BANK TRANSFER', '₱' . number_format($bankTransferAmount, 2)],
            ['', ''],
            ['SENIOR CITIZEN TRANSACTIONS', $seniorCitizenCount],
            ['TOTAL SENIOR DISCOUNT', '₱' . number_format($seniorDiscountTotal, 2)],
        ];
    }
