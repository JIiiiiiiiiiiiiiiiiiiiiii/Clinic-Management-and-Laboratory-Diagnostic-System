...
class RoleBasedAccess
{
    ...
    public function handle(Request $request, Closure $next, string $module, string $action = 'read'): Response
    {
        ...
        if (!$user->canAccessModule($module)) {
            return $this->handleUnauthorizedAccess($request, $user, $module);
        }

        // Check if user has the specific permission
        if (!$user->hasModulePermission($module, $action)) {
            return $this->handleUnauthorizedAccess($request, $user, $module, $action);
        }
     ...
     */
    private function handleUnauthorizedAccess(Request $request, $user, string $module, string $action = null): Response
    {
        $message = $action 
