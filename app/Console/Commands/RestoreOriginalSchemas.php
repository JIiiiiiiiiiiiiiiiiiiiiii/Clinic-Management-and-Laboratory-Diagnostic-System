...
class RestoreOriginalSchemas extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'lab:restore-original-schemas';
     ...
     * @var string
     */
    protected $description = 'Restore the original field schemas for the 3 main lab tests';
    ...
    {
        $this->info('Restoring Original Lab Test Schemas');
        $this->info('===================================');
        $this->newLine();
        ...
        foreach ($originalSchemas as $testCode => $schema) {
            $test = LabTest::where('code', $testCode)->first();
            
            if ($test) {
                $test->update(['fields_schema' => $schema]);
                $this->info("Restored original schema for {$test->name} ({$testCode})");
                $updatedCount++;
            } else {
                $this->warn("Test with code '{$testCode}' not found");
            }
        }

        $this->newLine();
        $this->info("Restored original schemas for {$updatedCount} lab tests");
