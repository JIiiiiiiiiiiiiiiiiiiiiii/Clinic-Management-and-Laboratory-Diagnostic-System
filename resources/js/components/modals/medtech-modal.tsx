...
import { Save, User, Mail, Microscope, CheckCircle, X, Lock } from 'lucide-react';
...
interface MedTechModalProps {
    isOpen: boolean;
    onClose: () => void;
    medtech?: MedTech | null;
    mode: 'create' | 'edit' | 'view';
    onSuccess?: () => void;
}

export default function MedTechModal({ 
    isOpen, 
    onClose, 
    medtech = null, 
    mode, 
    ...
    // Reset form when modal opens/closes or medtech changes
    useEffect(() => {
        if (isOpen) {
            if ((mode === 'edit' || mode === 'view') && medtech) {
                setData({
                    name: medtech.name || '',
                    email: medtech.email || '',
                    password: '',
                    ...
                    status: medtech.is_active ? 'Active' : 'Inactive',
                });
            } else if (mode === 'create') {
                reset();
            }
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        const url = mode === 'create' 
            ? '/admin/specialists/medtechs' 
            : `/admin/specialists/medtechs/${medtech?.id}`;
            ...
            <DialogContent className="sm:max-w-[600px] max-h-[90vh] overflow-y-auto">
                <DialogHeader>
                    <DialogTitle className="flex items-center gap-2 text-xl">
                        ...
                        {mode === 'create' ? 'Create New Med Tech' : mode === 'edit' ? 'Edit Med Tech' : 'View Med Tech'}
                    </DialogTitle>
                    <DialogDescription>
                        {mode === 'create' 
                            ? 'Add a new medical technologist to the clinic system' 
                            : mode === 'edit' 
                            ? 'Update medical technologist information'
                            : 'View medical technologist information'
                        }
                    </DialogDescription>
                </DialogHeader>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-4 border-t border-gray-200">
                            <div className="space-y-2">
                                ...
                                <div className="text-sm text-gray-800">
                                    {new Date(medtech.created_at).toLocaleDateString('en-US', {
                                        year: 'numeric',
                                        month: 'long',
                                        day: 'numeric',
                                        hour: '2-digit',
                            ...
                            <div className="space-y-2">
                                ...
                                <div className="text-sm text-gray-800">
                                    {new Date(medtech.updated_at).toLocaleDateString('en-US', {
                                        year: 'numeric',
                                        month: 'long',
                                        day: 'numeric',
                                        hour: '2-digit',
                            ...
                            <>
                                ...
                                >
                                    ...
                                    {isSubmitting 
                                        ...
                                        : (mode === 'create' ? 'Create Med Tech' : 'Update Med Tech')
                                    }
                                </Button>
                            </>
                        )}
