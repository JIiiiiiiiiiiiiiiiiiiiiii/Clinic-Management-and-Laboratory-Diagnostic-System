...
interface PatientBillingReceiptProps {
    user: {
        name: string;
        email: string;
        role: string;
...
export default function PatientBillingReceipt({
    user,
    patient,
    transaction,
    notifications = [],
    unreadCount = 0
}: PatientBillingReceiptProps) {
    const handlePrint = () => {
        window.print();
    };
                    ...
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-6">
                            ...
                            <div>
                                ...
                                <p className="text-gray-600">Receipt for {transaction.transaction_id}</p>
                            </div>
                        </div>
                                        ...
                                        <div className="space-y-2 text-sm">
                                            {transaction.patient ? (
                                                <>
                                                    ...
                                                    <div><span className="font-medium">Address:</span> {transaction.patient.present_address || 'N/A'}</div>
                                                    <div><span className="font-medium">Contact:</span> {transaction.patient.mobile_no || 'N/A'}</div>
                                                </>
                                            ) : (
                                                <>
                                            ...
                                            <tbody>
                                                {transaction.items.map((item, index) => (
                                                    <tr key={item.id} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                                                        ...
                                                        <td className="border border-gray-300 px-4 py-3 text-right">₱{Number(item.unit_price).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
                                                        <td className="border border-gray-300 px-4 py-3 text-right font-semibold">₱{Number(item.total_price).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
                                                    </tr>
                                                ))}
                                            </tbody>
                                        </table>
                                    ...
                                    <div className="w-full max-w-md">
                                        <div className="space-y-3">
                                            <div className="flex justify-between text-lg">
                                                ...
                                                <span className="font-semibold">₱{calculateSubtotal().toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                                            </div>
                                            ...
                                            {calculateDiscount() > 0 && (
                                                <div className="flex justify-between text-lg text-red-600">
                                                    ...
                                                    <span className="font-semibold">-₱{calculateDiscount().toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                                                </div>
                                            )}
                                            ...
                                            {calculateSeniorDiscount() > 0 && (
                                                <div className="flex justify-between text-lg text-red-600">
                                                    ...
                                                    <span className="font-semibold">-₱{calculateSeniorDiscount().toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                                                </div>
                                            )}
                                            ...
                                            <div className="border-t border-gray-300 pt-3">
                                                <div className="flex justify-between text-xl font-bold">
                                                    ...
                                                    <span>₱{calculateNetAmount().toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
