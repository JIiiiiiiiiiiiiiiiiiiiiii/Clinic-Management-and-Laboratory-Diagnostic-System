...
export default function LaboratoryReports({ user, labOrders, stats, dateRange, filters }: Props) {
  ...
  const handleFilter = () => {
    ...
    router.get(route('hospital.reports.laboratory'), Object.fromEntries(params), {
      preserveState: true,
      replace: true
    });
  };
  ...
  const handleExport = () => {
    ...
    window.location.href = `${route('hospital.reports.export', 'laboratory')}?${params.toString()}`;
  };
