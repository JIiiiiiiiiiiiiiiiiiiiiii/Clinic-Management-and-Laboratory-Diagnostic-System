...
import { Calendar, Download, Filter, MapPin, Phone, Search, User, Users } from 'lucide-react';
...
export default function HospitalPatientReports({ user, patients, stats, dateRange, filters }: Props) {
    ...
    const exportReport = () => {
        ...
        link.href = route('hospital.reports.export', 'patients') + '?' + params.toString();
        link.download = `patients_report_${new Date().toISOString().split('T')[0]}.csv`;
