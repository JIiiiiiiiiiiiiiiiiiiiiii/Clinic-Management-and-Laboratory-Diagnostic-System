...
const breadcrumbs = (doctor: Doctor): BreadcrumbItem[] => [
    ...
    {
        title: 'Doctors',
        href: '/admin/specialists/doctors',
    },
    {
        title: 'Edit Doctor',
        href: `/admin/specialists/doctors/${doctor.id}/edit`,
    },
];
...
export default function DoctorEdit({ doctor }: { doctor: Doctor }) {
    ...
    const submit: React.FormEventHandler = (e) => {
        e.preventDefault();
        router.put(`/admin/specialists/doctors/${doctor.id}`, data, {
            onSuccess: () => {
                router.visit('/admin/specialists/doctors');
            },
            onError: (errors) => {
                ...
                <div className="mb-8">
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-6">
                            <div>
                                <h1 className="text-4xl font-semibold text-black mb-4">Edit Doctor</h1>
                                <p className="text-sm text-black mt-1">Update doctor information - {doctor.name}</p>
                            </div>
                        </div>
                            ...
                            <div className="bg-white rounded-xl shadow-lg border px-6 py-4">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <div className="text-black text-sm font-medium">Doctor Profile</div>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <CardContent className="p-6">
                            <form onSubmit={submit} className="space-y-6">
                                ...
                                <div className="bg-gray-50 p-4 rounded-lg">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                        <div className="p-3 bg-white rounded-lg border">
                                            ...
                                            <Badge className={doctor.is_active ? 'bg-gray-100 text-black' : 'bg-gray-100 text-gray-800'}>
                                                {doctor.is_active ? 'Active' : 'Inactive'}
                                            </Badge>
                                        </div>
                                        <div className="p-3 bg-white rounded-lg border">
                                            <div className="text-sm font-medium text-gray-600 mb-1">Created</div>
                                            <div className="text-lg font-bold text-black">{new Date(doctor.created_at).toLocaleDateString()}</div>
                                        </div>
                                        <div className="p-3 bg-white rounded-lg border">
                                            <div className="text-sm font-medium text-gray-600 mb-1">Last Updated</div>
                                            <div className="text-lg font-bold text-black">{new Date(doctor.updated_at).toLocaleDateString()}</div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="flex justify-end gap-4 pt-6 border-t border-gray-200">
                                    <Link href="/admin/specialists/doctors">
                                        <Button
                                            type="button"
                                            variant="outline"
