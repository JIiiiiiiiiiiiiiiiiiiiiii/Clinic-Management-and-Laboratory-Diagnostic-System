...
return new class extends Migration
{
    ...
    {
        // First, drop all foreign key constraints that might prevent column drops
        DB::statement('SET FOREIGN_KEY_CHECKS=0;');

        // Drop foreign key constraints on billing_transactions
        try {
        ...
        try {
            DB::statement('ALTER TABLE billing_transactions DROP FOREIGN KEY billing_transactions_specialist_id_foreign');
        } catch (Exception $e) {
            // Foreign key might not exist
        }

        try {
            DB::statement('ALTER TABLE billing_transactions DROP FOREIGN KEY billing_transactions_appointment_id_foreign');
        } catch (Exception $e) {
            // Foreign key might not exist
        }

        // Drop foreign key constraints on appointments
        try {
            DB::statement('ALTER TABLE appointments DROP FOREIGN KEY appointments_patient_id_fk_foreign');
        } catch (Exception $e) {
            // Foreign key might not exist
        }

        try {
            DB::statement('ALTER TABLE appointments DROP FOREIGN KEY appointments_specialist_id_fk_foreign');
        } catch (Exception $e) {
            // Foreign key might not exist
        }
