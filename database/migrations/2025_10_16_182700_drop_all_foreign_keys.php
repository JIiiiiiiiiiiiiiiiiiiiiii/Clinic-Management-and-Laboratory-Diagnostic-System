...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        // Get all foreign key constraints for our tables
        $constraints = DB::select("
            SELECT CONSTRAINT_NAME, TABLE_NAME, COLUMN_NAME 
            FROM information_schema.KEY_COLUMN_USAGE 
            WHERE REFERENCED_TABLE_SCHEMA = DATABASE() 
            AND TABLE_NAME IN ('appointments', 'billing_transactions', 'visits')
        ...
        foreach ($constraints as $constraint) {
            try {
                ...
                echo "Dropped foreign key: {$constraint->CONSTRAINT_NAME} from {$constraint->TABLE_NAME}\n";
            } catch (Exception $e) {
                echo "Could not drop {$constraint->CONSTRAINT_NAME}: " . $e->getMessage() . "\n";
            }
        }
